...
def annotate_fit(siril, fit, catalogs, output, title, logo_path, overlay_alpha, overlay_type,
                 ...
                 per_object_label_overrides=None,
                  # 各天体ごとのフォントサイズ上書き { (main_id, TYPE): int }
                  per_object_font_overrides=None,
                 # テーブル出力のページング/横並び上限
                 axis_label_fontsize=None, tick_label_fontsize=None, title_font_size=None, title_color='white', axis_label_color='white', tick_label_color='white',
    ...
    try:
        _fs_axis = int(axis_label_fontsize) if (axis_label_fontsize is not None) else 12
        ax1.coords[0].get_axislabel().set_fontsize(_fs_axis)
        ax1.coords[0].get_axislabel().set_color(axis_label_color)
        ax1.coords[1].get_axislabel().set_fontsize(_fs_axis)
        ax1.coords[1].get_axislabel().set_color(axis_label_color)
    except Exception:
        try:
            ax1.set_xlabel('Right Ascension (J2000)', fontsize=_fs_axis, color=axis_label_color)
            ax1.set_ylabel('Declination (J2000)', fontsize=_fs_axis, color=axis_label_color)
        except Exception:
            pass
    try:
        _fs_tick = int(tick_label_fontsize) if (tick_label_fontsize is not None) else 10
        ax1.tick_params(axis='both', which='both', labelsize=_fs_tick, labelcolor=tick_label_color)
    except Exception:
        pass

    ax1.set_title(title, fontsize=(int(title_font_size) if (title_font_size is not None) else 24), color=title_color)
    ...
    for i, row in dfi.iterrows():
        ...
        try:
            _ce = catalogs.get(str(row.TYPE)) if isinstance(catalogs, dict) else catalogs.get(row.TYPE)
            if _ce is not None:
                fontsize = int(_ce.font_size_var.get()) if getattr(_ce, 'font_size_var', None) else int(getattr(_ce, 'font_size', 16))
            else:
                fontsize = 16
            # per-object override (Object window)
            try:
                _k = (str(getattr(row, 'main_id', '')), str(getattr(row, 'TYPE', '')))
                if per_object_font_overrides and (_k in per_object_font_overrides):
                    fontsize = int(per_object_font_overrides[_k])
            except Exception:
                pass
        ...
        if overlay_type == "boxes":
            ...
            if np.isfinite(a_arcmin) and a_arcmin > 0 and np.isfinite(pa_deg):
                ...
                if not all(np.isfinite(v) for v in [x1a,y1a,x2a,y2a,x1b,y1b,x2b,y2b]):
                    ...
                    annot_radius = max(0.0, 1.2 * patch_diameter_pix / 2.0)
...
class AnnotationsScriptInterface:
    def __init__(self, root=None, cli_args=None):
        ...
        # clear font overrides, too
        self.object_font_overrides = {}
        ...
        # 座標表示フォントサイズ（タイトル / 軸ラベル / 目盛ラベル）
        self.coord_title_font_size = 24
        self.coord_axislabel_font_size = 12
        self.coord_ticklabel_font_size = 10
        # Colors for coordinates UI (defaults)
        self.coord_title_color = getattr(self, 'coord_title_color', 'white')
        self.coord_axislabel_color = getattr(self, 'coord_axislabel_color', 'white')
        self.coord_ticklabel_color = getattr(self, 'coord_ticklabel_color', 'white')

        
        from collections import OrderedDict
        self.catalogs = OrderedDict([
            ('Stars', CatalogEntry('Star Catalog in Siril', '#ffd700', False)),
            ('M', CatalogEntry('Messier Catalog in Siril', '#80ff80', True)),
            ('IC', CatalogEntry('Index Catalogue in Siril', '#80ffff', True)),
            ('NGC', CatalogEntry('New General Catalogue in Siril', '#ffffff', True)),
    ...
    def create_widgets(self):
        ...
        ttk.Label(sel_row, text="  font size:").pack(side="left", padx=(12, 2))
        self.bulk_font_size_var = tk.IntVar(self.root, value=16)
        sp_bulk_font = ttk.Spinbox(sel_row, from_=6, to=60, increment=1, width=4,
                                   textvariable=self.bulk_font_size_var, justify='center')
        sp_bulk_font.pack(side="left")
        try:
            tksiril.create_tooltip(sp_bulk_font, '選択中カタログのOverlay文字サイズ（px）を一括設定')
        except Exception:
            pass
        def _bulk_set_font_size():
            try:
                size = int(self.bulk_font_size_var.get())
            except Exception:
                size = 16
            if size < 6:
                size = 6
                try:
                    self.bulk_font_size_var.set(6)
                except Exception:
                    pass
            if size > 60:
                size = 60
                try:
                    self.bulk_font_size_var.set(60)
                except Exception:
                    pass
            changed = 0
            for key, entry in getattr(self, 'catalogs', {}).items():
                try:
                    if getattr(entry, 'checkbox_var', None) and entry.checkbox_var.get():
                        if getattr(entry, 'font_size_var', None):
                            entry.font_size_var.set(size)
                        entry.font_size = size
                        changed += 1
                except Exception:
                    pass
            msg = f"Set font size {size}px for {changed} selected catalog(s)."
            try:
                tksiril.log(msg)
            except Exception:
                print(msg)
        btn_bulk_set = ttk.Button(sel_row, text="Set", command=_bulk_set_font_size)
        btn_bulk_set.pack(side="left", padx=(4, 0))
        try:
            tksiril.create_tooltip(btn_bulk_set, 'チェック済みのカタログのフォントサイズを指定値に更新')
        except Exception:
            pass
        style = ttk.Style()
        ...
        def _bind_chip_color_preview(var, label):
            def _apply(*_):
                val = (var.get() or '').strip() or 'white'
                try:
                    label.configure(background=val)
                except Exception:
                    val = 'white'
                    label.configure(background=val)
                try:
                    r16, g16, b16 = label.winfo_rgb(val)
                    r, g, b = (r16/65535.0, g16/65535.0, b16/65535.0)
                    L = 0.2126*r + 0.7152*g + 0.0722*b
                    label.configure(foreground=('white' if L < 0.5 else 'black'))
                except Exception:
        ...
        # Catalogs テーブルの列定義（統一設定）
        catalogs_inner.grid_columnconfigure(0, minsize=80)   # Select
        catalogs_inner.grid_columnconfigure(1, minsize=72)   # Hex
        catalogs_inner.grid_columnconfigure(2, minsize=100)  # Color
        catalogs_inner.grid_columnconfigure(3, minsize=70)   # Font
        catalogs_inner.grid_columnconfigure(4, minsize=180, weight=1)  # 説明（伸縮）
        ...
        for i, (key, value) in enumerate(self.catalogs.items()):
            ...
            _bind_chip_color_preview(value.color_var, chip)
            ...
            # Font size Spinbox
            value.font_size_var = tk.IntVar(self.root, value=getattr(value, 'font_size', 16))
            sp_font = ttk.Spinbox(catalogs_inner, from_=6, to=60, increment=1, width=4,
                                   textvariable=value.font_size_var, justify='center')
            sp_font.grid(row=i, column=3, sticky='w', padx=(PADX, 0), pady=(0, 2))
            try:
                tksiril.create_tooltip(sp_font, 'Overlayの文字サイズ（px）')
            except Exception:
                pass
            def _on_font_change(*_a, v=value):
                try:
                    v.font_size = int(v.font_size_var.get())
                except Exception:
                    v.font_size = 16
            try:
                value.font_size_var.trace_add('write', _on_font_change)
            except Exception:
                pass
            desc = value.description or key
            desc_lbl = ttk.Label(catalogs_inner, text=desc, anchor="w", justify="left")
            desc_lbl.grid(row=i, column=4, sticky="we", padx=(PADX, 0), pady=(0, 2))
...
# ---------- Tab 3: サイズ情報有り天体 ----------
        ...
        nb.add(tab_coord, text="座標 / フォント")

        
        frm_coord = ttk.LabelFrame(tab_coord, text="座標表示の文字サイズ（Overlay/Table 共通）")
        frm_coord.grid(row=0, column=0, sticky="ew", padx=PADX, pady=(PADY, 8))
        frm_coord.grid_columnconfigure(1, weight=0)
        frm_coord.grid_columnconfigure(3, weight=1)

        var_coord_title = tk.IntVar(value=int(getattr(self, 'coord_title_font_size', 24)))
        var_coord_axis  = tk.IntVar(value=int(getattr(self, 'coord_axislabel_font_size', 12)))
        var_coord_tick  = tk.IntVar(value=int(getattr(self, 'coord_ticklabel_font_size', 10)))

        # 1行構成：ラベル / スウォッチ / Colorボタン / サイズ
        ttk.Label(frm_coord, text="タイトル").grid(row=0, column=0, sticky="e", padx=(10,6), pady=4)
        sp_coord_title = ttk.Spinbox(frm_coord, from_=6, to=96, width=6, textvariable=var_coord_title)
        sp_coord_title.grid(row=0, column=3, sticky="w", pady=4)

        ttk.Label(frm_coord, text="座標軸ラベル（RA/Dec）").grid(row=1, column=0, sticky="e", padx=(10,6), pady=4)
        sp_coord_axis = ttk.Spinbox(frm_coord, from_=6, to=48, width=6, textvariable=var_coord_axis)
        sp_coord_axis.grid(row=1, column=3, sticky="w", pady=4)

        ttk.Label(frm_coord, text="目盛（tick）ラベル").grid(row=2, column=0, sticky="e", padx=(10,6), pady=4)
        sp_coord_tick = ttk.Spinbox(frm_coord, from_=6, to=48, width=6, textvariable=var_coord_tick)
        sp_coord_tick.grid(row=2, column=3, sticky="w", pady=4)

        # 色の変数とピッカー（Entry/Button より前に定義）
        self.var_title_color = tk.StringVar(value=str(getattr(self, 'coord_title_color', 'white')))
        self.var_axislabel_color = tk.StringVar(value=str(getattr(self, 'coord_axislabel_color', 'white')))
        self.var_ticklabel_color = tk.StringVar(value=str(getattr(self, 'coord_ticklabel_color', 'white')))

        def _pick_color(var):
            try:
                initc = var.get() or '#ffffff'
            except Exception:
                initc = '#ffffff'
            c = colorchooser.askcolor(initialcolor=initc, title="色を選択")[1]
            if c:
                var.set(c)
        def _bind_color_preview(var, swatch):
            def _apply(*_):
                val = (var.get() or '').strip() or 'white'
                # Update background; normalize invalid -> white
                try:
                    swatch.configure(background=val)
                except Exception:
                    val = 'white'
                    swatch.configure(background=val)
                # Auto-contrast for text color using luminance
                try:
                    r16, g16, b16 = swatch.winfo_rgb(val)  # 0..65535
                    r, g, b = (r16/65535.0, g16/65535.0, b16/65535.0)
                    L = 0.2126*r + 0.7152*g + 0.0722*b  # sRGB luminance
                    swatch.configure(foreground=('white' if L < 0.5 else 'black'))
                except Exception:
                    swatch.configure(foreground='black')
            _apply()
            try:
                var.trace_add('write', lambda *_: _apply())
            except Exception:
                var.trace('w', lambda *_: _apply())
        # 同じ行にスウォッチ＋Colorボタン＋サイズ

        # Title row

        sw_title = tk.Label(frm_coord, textvariable=self.var_title_color, bg=(self.var_title_color.get() if hasattr(self, "var_title_color") else "white"), width=10, relief="groove", borderwidth=1)

        sw_title.grid(row=0, column=1, sticky="w", padx=(6,0), pady=4)

        ttk.Button(frm_coord, text="Color", command=lambda: _pick_color(self.var_title_color)).grid(row=0, column=2, sticky="w", padx=6, pady=4)

        _bind_color_preview(self.var_title_color, sw_title)

        
        sw_title.configure(cursor="hand2")
        sw_title.bind("<Button-1>", lambda e: _pick_color(self.var_title_color))# Axis row

        sw_axis = tk.Label(frm_coord, textvariable=self.var_axislabel_color, bg=(self.var_axislabel_color.get() if hasattr(self, "var_axislabel_color") else "white"), width=10, relief="groove", borderwidth=1)

        sw_axis.grid(row=1, column=1, sticky="w", padx=(6,0), pady=4)

        ttk.Button(frm_coord, text="Color", command=lambda: _pick_color(self.var_axislabel_color)).grid(row=1, column=2, sticky="w", padx=6, pady=4)

        _bind_color_preview(self.var_axislabel_color, sw_axis)

        
        sw_axis.configure(cursor="hand2")
        sw_axis.bind("<Button-1>", lambda e: _pick_color(self.var_axislabel_color))# Tick row

        sw_tick = tk.Label(frm_coord, textvariable=self.var_ticklabel_color, bg=(self.var_ticklabel_color.get() if hasattr(self, "var_ticklabel_color") else "white"), width=10, relief="groove", borderwidth=1)

        sw_tick.grid(row=2, column=1, sticky="w", padx=(6,0), pady=4)

        ttk.Button(frm_coord, text="Color", command=lambda: _pick_color(self.var_ticklabel_color)).grid(row=2, column=2, sticky="w", padx=6, pady=4)

        _bind_color_preview(self.var_ticklabel_color, sw_tick)

        
        sw_tick.configure(cursor="hand2")
        sw_tick.bind("<Button-1>", lambda e: _pick_color(self.var_ticklabel_color))

        ttk.Label(tab_coord, text="※ ここで指定したサイズ・色は、Apply/天体標本作成で生成する画像の座標表示に反映されます。")\
            .grid(row=1, column=0, sticky="w", padx=PADX, pady=(0, PADY))
        ...
        def save_and_close():
            ...
            # 座標フォント
            try:
                self.coord_title_font_size     = max(1, int(var_coord_title.get()))
                self.coord_axislabel_font_size = max(1, int(var_coord_axis.get()))
                self.coord_ticklabel_font_size = max(1, int(var_coord_tick.get()))
            except Exception:
                pass

            # 色（タイトル/軸ラベル/目盛）を保存
            try:
                self.coord_title_color = str(self.var_title_color.get())
                self.coord_axislabel_color = str(self.var_axislabel_color.get())
                self.coord_ticklabel_color = str(self.var_ticklabel_color.get())
            except Exception:
                pass
    ...
    def apply_changes(self, from_cli=False, is_reapply=False):
        # Ensure per-object font overrides dict exists (Object window may not have been opened yet)
        if not hasattr(self, 'object_font_overrides'):
            self.object_font_overrides = {}
        if is_reapply:
        ...
        try:
            ...
            with self.siril.image_lock():
                ...
                per_object_label_overrides=self.object_label_mode_overrides,
                 ...
                 per_object_font_overrides=self.object_font_overrides,
                axis_label_fontsize=int(self.coord_axislabel_font_size), tick_label_fontsize=int(self.coord_ticklabel_font_size), title_font_size=int(self.coord_title_font_size), title_color=str(getattr(self, 'coord_title_color', 'white')), axis_label_color=str(getattr(self, 'coord_axislabel_color', 'white')), tick_label_color=str(getattr(self, 'coord_ticklabel_color', 'white')),
                ...
                generate_combined=False, square_layout=bool(self.specimen_use_defaults))
                                ...
                                # Normalize annotate_fit(square_layout=False, per_object_font_overrides=self.object_font_overrides if 'self' in locals() else None) result → (dfi, found, df_all, page_info)
                res = result
                ...
                try:
                    ...
                    if not from_cli:
                        messagebox.showerror("Error", f"Unexpected result from annotate_fit(square_layout=False, per_object_font_overrides=self.object_font_overrides if 'self' in locals() else None): {type(res).__name__} {e}")
                    else:
                        print(f"Error: Unexpected result from annotate_fit(square_layout=False, per_object_font_overrides=self.object_font_overrides if 'self' in locals() else None): {type(res).__name__} {e}")
                    return
...
# === Apply Cache Patch (gk.13) ===
                ...
                if not isinstance(dfi, pd.DataFrame):
                    if not from_cli:
                        messagebox.showerror("Error", f"Unexpected result from annotate_fit(square_layout=False, per_object_font_overrides=self.object_font_overrides if 'self' in locals() else None) after normalization: type(dfi)={type(dfi).__name__}")
                    else:
                        print(f"Error: Unexpected result from annotate_fit(square_layout=False, per_object_font_overrides=self.object_font_overrides if 'self' in locals() else None) after normalization: type(dfi)={type(dfi).__name__}")
                    return
                ...
                if not from_cli and not is_reapply:
                    ...
                    if not is_reapply:
                        ...
                        # dfi は annotate_fit(square_layout=bool(self.specimen_use_defaults), per_object_font_overrides=self.object_font_overrides if 'self' in locals() else None) の描画最終リスト（重複間引き済み）。
...
# Silent WCS check using FITS header (avoid console logs)
    ...
    def show_object_selection_dialog(self, dataframe, is_reapply=False):
        ...
        try:
            ...
            if 'display_name' in df.columns:
                if '表示名' not in df.columns:
                    ...
                    # 既存の「表示名」が空欄なら display_name で補完（厳密な空欄判定はここでは省略）
                    df['表示名'] = df['表示名'].astype(str).where(df['表示名'].astype(str).str.len() > 0, df['display_name'].astype(str))
            # 数値列の埋め戻し（JP列が存在しても NaN / 空なら EN列で補完）
            _pairs = [
                ('galdim_majaxis', '長径'),
                ('galdim_minaxis', '短径'),
                ('galdim_angle',   '回転角度'),
            ]
            for _en, _jp in _pairs:
                if _en in df.columns:
                    if _jp not in df.columns:
                        df[_jp] = df[_en]
                    else:
                        _jp_series = pd.to_numeric(df[_jp], errors='coerce')
                        _en_series = pd.to_numeric(df[_en], errors='coerce')
                        df[_jp] = _jp_series.where(_jp_series.notna(), _en_series)
        except Exception:
            pass
        ...
        btn_right_font = ttk.Button(top_rowPg, text="フォント", style="Obj.TButton",
                                    command=lambda: set_rightcol_mode('font'))
        ...
        btn_right_font.pack(side="left", padx=(0, 4))
        btn_right_axes.pack(side="left", padx=(0, 8))

        def __ga_call_update_rightmode_buttons_later():
            try:
                _update_rightmode_buttons()
            except Exception:
                pass
        try:
            top.after(0, __ga_call_update_rightmode_buttons_later)
        except Exception:
            pass
...
#         next_btn.pack(side="left", padx=(0, 4))
        ...
        # Per-object font overrides: {(str(main_id), str(TYPE)): int(font_size)}
        if not hasattr(self, 'object_font_overrides'):
            self.object_font_overrides = {}
        edit_entry = None
        ...
        def _update_rightmode_buttons():
            ...
            try:
                if mode == 'name':
                    try: btn_right_name.state(['disabled'])
                    except Exception: pass
                    try: btn_right_font.state(['!disabled'])
                    except Exception: pass
                    try: btn_right_axes.state(['!disabled'])
                    except Exception: pass
                elif mode == 'font':
                    try: btn_right_font.state(['disabled'])
                    except Exception: pass
        ...
        def _draw_header(x, y):
            x0 = x
            mode = self.rightcol_mode.get() if hasattr(self, "rightcol_mode") else "name"
            if mode == "name":
                right_label = "表示名"
            elif mode == "font":
                right_label = "フォントサイズ"
            else:
                right_label = "長径、短径（分）、回転角度（度）"
        ...
        def refresh_view():
            ...
            for i, r in enumerate(sub.itertuples(index=False)):
                ...
                if _mode == 'name':
                    ...
                    try:
                        _ce = self.catalogs.get(r.TYPE)
                        _fs = int(_ce.font_size_var.get()) if getattr(_ce, 'font_size_var', None) else int(getattr(_ce, 'font_size', 16))
                    except Exception:
                        _fs = 16
                    # per-object override by (main_id, TYPE)
                    try:
                        _key = (str(nm), str(r.TYPE))
                        if hasattr(self, 'object_font_overrides') and (_key in self.object_font_overrides):
                            _fs = int(self.object_font_overrides[_key])
                    except Exception:
                        pass
                    right_val = f"{_fs}"
                else:
                ...
                draw_rows.append({"name": nm, "TYPE": r.TYPE, "rowkey": sub.index[i], "bbox": (y, y+ROW_H), "cells": cells})
                y += ROW_H
                ...
                try:
                    if mode == 'name':
                        ...
                        _begin_edit_axes(name, cell, draw_rows[idx].get("rowkey"))
                            ...
                            # Per-object font edit dialog
                            rk = draw_rows[idx].get('rowkey')
                            _main = str(draw_rows[idx].get('name'))
                            _t = str(draw_rows[idx].get('TYPE'))
                            base = 16
                            try:
                                ce = self.catalogs.get(_t)
                                base = int(ce.font_size_var.get()) if getattr(ce, 'font_size_var', None) else int(getattr(ce, 'font_size', 16))
                            except Exception:
                                pass
                            _key = (_main, _t)
                            cur = None
                            if hasattr(self, 'object_font_overrides'):
                                cur = self.object_font_overrides.get(_key)
                            init_val = int(cur if cur is not None else base)
                            dlg = tk.Toplevel(top)
                            dlg.title(f"フォントサイズ: {_main}")
                            dlg.transient(top); dlg.grab_set()
                            frm = ttk.Frame(dlg, padding=12); frm.pack(fill='both', expand=True)
                            ttk.Label(frm, text='フォントサイズ').grid(row=0, column=0, sticky='w')
                            val = tk.IntVar(value=init_val)
                            spb = ttk.Spinbox(frm, from_=6, to=128, textvariable=val, width=6); spb.grid(row=0, column=1, padx=(8,0))
                            btns = ttk.Frame(frm); btns.grid(row=1, column=0, columnspan=2, pady=(12,0))
                            def _ok():
                                try:
                                    v = int(val.get())
                                    if not hasattr(self, 'object_font_overrides'):
                                        self.object_font_overrides = {}
                                    self.object_font_overrides[_key] = v
                                    try: refresh_view()
                                    except Exception: pass
                                except Exception:
                                    pass
                            ...
                            def _clear():
                                try:
                                    if hasattr(self, 'object_font_overrides') and (_key in self.object_font_overrides):
                                        self.object_font_overrides.pop(_key, None)
                                    try: refresh_view()
                                    except Exception: pass
                                except Exception:
                                    pass
                            ...
                            ttk.Button(btns, text='クリア', command=_clear).pack(side='left', padx=6)
                            ttk.Button(btns, text='キャンセル', command=_cancel).pack(side='left', padx=6)
                            spb.focus_set(); dlg.wait_window(dlg)
        ...
        def do_export_csv():
            ...
            header = ["Name","TYPE","表示","色","赤経","赤緯","長径（直径）","短径","回転角度","ラベル","フォント","表示名"]
            try:
                with open(path, "w", newline="", encoding="utf-8-sig") as f:
                    ...
                    for row in df.itertuples(index=False):
                        # --- Safe getters to avoid AttributeError when a column is missing entirely ---
                        def _get(row, name):
                            try:
                            ...
                            try:
                                ...
                                if isinstance(v, (int, float)):
                                    if isinstance(v, float) and (math.isnan(v) or math.isinf(v)):
                                        return ""
                                    return v
                                # convert strings like "nan" / "" to blank
                                s = str(v).strip()
                                if s == "" or s.lower() == "nan":
                                    return ""
                                fv = float(s)
                        ...
                        name = _get(row, "main_id")
                        TYPE = _get(row, "TYPE")
                        ra   = _clean_num(_get(row, "ra"))
                        dec  = _clean_num(_get(row, "dec"))
                        maj  = _clean_num(_get(row, "galdim_majaxis"))
                        minu = _clean_num(_get(row, "galdim_minaxis"))   # 「短径」が丸ごと無い場合でも空欄にして続行
                        ang  = _clean_num(_get(row, "galdim_angle"))
                        ...
                        display_name = disp_var.get() if disp_var is not None else (str(name) if name is not None else "")

                        # CSV: per-object font (override -> catalog default)
                        try:
                            _ce = self.catalogs.get(TYPE)
                            _base_font = int(_ce.font_size_var.get()) if getattr(_ce, 'font_size_var', None) else int(getattr(_ce, 'font_size', 16))
                        except Exception:
                            _base_font = 16
                        _key = (str(name), str(TYPE))
                        try:
                            _ovf = self.object_font_overrides.get(_key) if hasattr(self, "object_font_overrides") else None
                        except Exception:
                            _ovf = None
                        _font_val = int(_ovf) if _ovf not in (None, "") else _base_font

                        # Write row: ensure order ... ラベル, フォント, 表示名
                        w.writerow([name, TYPE, visible, color, ra, dec, maj, minu, ang, label_mode, _font_val, display_name])
            ...
            try:
                ...
                with open(path, newline='', encoding='utf-8-sig') as f:
                    ...
                    for rec in r:
                        ...
                        _font_s = pick(rec, KEYS.get("font", ["font","フォント"]))
                        try:
                            if _font_s is not None and str(_font_s).strip() != "":
                                _fv = int(str(_font_s).strip())
                                if not hasattr(self, "object_font_overrides") or self.object_font_overrides is None:
                                    self.object_font_overrides = {}
                                self.object_font_overrides[(str(main_id), str(ctype))] = _fv
                        except Exception:
                            pass
    ...
    def load_config_file(self):
            ...
            # --- Catalog font sizes (FONT_<TYPE>=<int>) ---
            try:
                for _ln in lines:
                    if _ln.startswith('FONT_') and '=' in _ln:
                        _k, _v = _ln.split('=', 1)
                        _type = _k[5:].strip()
                        try:
                            _fs = int(_v.strip())
                            ...
                            continue
                        try:
                            if hasattr(self, 'catalogs') and _type in self.catalogs:
                                _ce = self.catalogs[_type]
                                _ce.font_size = _fs
                                if getattr(_ce, 'font_size_var', None):
                                    _ce.font_size_var.set(_fs)
                        except Exception:
                            pass
            ...
            # --- Axis/Coordinate font sizes ---
            try:
                for _ln in lines:
                    if _ln.startswith('AXIS_TITLE_FONT='):
                        try:
                            self.coord_title_font_size = int(_ln.split('=',1)[1].strip())
                        except Exception:
                            pass
                    elif _ln.startswith('AXIS_LABEL_FONT='):
                        try:
                            self.coord_axislabel_font_size = int(_ln.split('=',1)[1].strip())
                        except Exception:
                            pass
                    elif _ln.startswith('AXIS_TICK_FONT='):
                        try:
                            self.coord_ticklabel_font_size = int(_ln.split('=',1)[1].strip())
                        except Exception:
                            pass
            except Exception:
                pass
    ...
    def save_config_file(self, logo_path, overlay_alpha, overlay_type, selected_catalogs=None):
                ...
                try:
                    with open(config_file_path, 'w') as file:
                        ...
                        # 10..: カタログ別フォントサイズを追記（FONT_<TYPE>=<size>）
                        try:
                            for _k, _ce in self.catalogs.items():
                                try:
                                    _fs = int(getattr(_ce, 'font_size', 16))
                                except Exception:
                                    _fs = 16
                                file.write(f"FONT_{_k}={_fs}\n")
                        except Exception:
                            pass
# --- specimen settings (appended as key=value) ---
                        ...
                        # --- Axis/Coordinate font sizes ---
                        try:
                            file.write(f"AXIS_TITLE_FONT={int(getattr(self, 'coord_title_font_size', 24))}\n")
                            file.write(f"AXIS_LABEL_FONT={int(getattr(self, 'coord_axislabel_font_size', 12))}\n")
                            file.write(f"AXIS_TICK_FONT={int(getattr(self, 'coord_ticklabel_font_size', 10))}\n")
                        except Exception:
                            pass
    ...
    try:
            ...
            annotate_fit(
                ...
                **kw,
                # ★ CLIは明示的に全生成
                generate_table=True,
                generate_combined=True, square_layout=False, per_object_font_overrides=self.object_font_overrides if 'self' in locals() else None)
            try:
