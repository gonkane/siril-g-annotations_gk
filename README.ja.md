# Siril用 Galaxy Annotationsスクリプト（バージョン 1.0.2-gk.2、作成者 gonkane）
...
の中に元の `Galaxy_Annotations.py` があるので、そこに `Galaxy_Annotations_102gk2.py` を置いておくだけでも問題ありません。
...
2. `Galaxy_Annotations_102gk2.py` を選んで実行  
...
### 最新バージョン：[Galaxy_Annotations_102gk2.py](Galaxy_Annotations_102gk2.py)
- バージョン: 1.0.2-gk.2  
- RA±・DEC± の 4方向評価に基づくアノテーションサイズの計算を改良  
- サイズ情報のない天体へのフォールバック処理を追加  
- 現在はこちらの使用を推奨します

### 旧バージョン：[Galaxy_Annotations_102gk1.py](Galaxy_Annotations_102gk1.py)
- バージョン: 1.0.2-gk.1  
- バージョン 1.0.2-gk.2 に置き換えられました  
- 参考用・過去互換のためにのみ保持されています
