...
#
# Customized by: gonkane, 2025 — version 1.0.2-gk.2
# See version history below for details.
#
# License: GPL v3 or later (see LICENSE file for details)

"""
Galaxy Annotation Script for Siril (Custom Version 1.0.2-gk.2 by gonkane)
...
def annotate_fit(siril, fit, catalogs, output, title, logo_path, overlay_alpha, overlay_type):
    ...
    def safe_radec2pix(row):
        try:
            return siril.radec2pix(row['ra'], row['dec'])
        except Exception as e:
    ...
    df = df.dropna(subset=['Pixel_Position'])
    ...
    for i, row in dfi.iterrows():
        ...
        if angular_size == 0:
            ...
            try:
                ...
                siril.log(f"{row.main_id}: WCS radius estimation failed: {e}", color=s.LogColor.YELLOW)
                patch_size = min_patch_size
                patch_diameter_pix = min_patch_size
