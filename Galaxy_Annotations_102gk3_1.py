...
#
# Customized by: gonkane, 2025 — version 1.0.2-gk.3.1
# See version history below for details.
#
# License: GPL v3 or later (see LICENSE file for details)

"""
Galaxy Annotation Script for Siril (Custom Version 1.0.2-gk.3.1 by gonkane)
...
- Built-in catalog support (Messier, NGC, IC)
- Simbad and local annotation integration
- Per-object control of color and visibility
- Enhanced GUI with object selector and image switching (C/O/T/N)
- For objects with identical center coordinates,
  annotations (circles/boxes) are shifted outward to avoid overlap

"""
# Version History
# For complete version history and release notes, visit:
# https://github.com/gonkane/siril-g-annotations_gk/releases
...
# custom_object_colorsを追加
def annotate_fit(siril, fit, catalogs, output, title, logo_path, overlay_alpha, overlay_type, custom_object_colors, visible_object_names=None):
    ...
    # ここで「チェックonの天体」のみに絞る
    if visible_object_names is not None:
        dfi = dfi[dfi['main_id'].isin(visible_object_names)]
        print(f"Filtered by visibility: {dfi.shape[0]} entries")

    # === チェックされた天体に対してのみ重複インデックスを計算 ===
    ...
    for i, row in dfi.iterrows():
        ...
        if overlay_type == "boxes":
            # --- duplicate_index に応じて拡大 ---
            size_increment = 60  # ピクセル単位の拡大量（3でも可）
            expansion = 0
            if hasattr(row, 'duplicate_index'):
                expansion = row.duplicate_index * size_increment

            # 新しいサイズに拡大（上下左右に expansion 分ずつ）
            x1_exp = x1 - expansion
            x2_exp = x2 + expansion
            y1_exp = y1 - expansion
            y2_exp = y2 + expansion
            ...
            # annotation circle
            radius_increment = 60.0  # 重複ごとに半径を増やす
    ...
    for i, row in dft.iterrows():
        ...
        # custom_object_colors があればそれを優先
        if row.main_id in custom_object_colors:
            color = custom_object_colors[row.main_id]
        else:
            try:
    ...
    return dfi, dfi.shape[0], df  # df（全天体データフレーム）
...
class AnnotationsScriptInterface:
    ...
    def __init__(self, root=None, cli_args=None):
        ...
        self.custom_object_colors = {}  # 天体ごとの色設定

        self.visible_object_flags = {}  # 天体ごとの表示ON/OFF状態を記録
    ...
    def create_widgets(self):
        ...
        # CanvasとScrollbarを作成
        canvas = tk.Canvas(catalogs_frame, height=210)  # 高さを制限（必要に応じて調整）
    ...
    def show_or_focus_object_window(self):
        ...
        # 開いていなければリストを渡して開く
            ...
            self.show_object_selection_dialog(self.df_all)
        else:
    ...
    def apply_changes(self, from_cli=False, is_reapply=False):
        ...
        try:
            ...
            if not from_cli and not is_reapply and hasattr(self, 'df_all') and self.df_all is not None:
                ...
                # OKが押された場合、Change Object Colors ウィンドウが開いていれば閉じる
                if hasattr(self, 'object_control_window') and self.object_control_window.winfo_exists():
                    self.object_control_window.destroy()
                
            # --- 元画像を読み込む（ReApplyと同様に） ---
            ...
            with self.siril.image_lock():
                ...
                if not from_cli and not is_reapply:
                    ...
                    if hasattr(self, 'df_all') and self.df_all is not None:
                        for row in self.df_all.itertuples(index=False):
                            name = row.main_id
                            ctype = row.TYPE
                            if ctype in self.catalogs and self.catalogs[ctype].get_selected():
                                self.visible_object_flags[name] = tk.BooleanVar(value=True)
                                self.custom_object_colors[name] = self.catalogs[ctype].color
                            else:
                                self.visible_object_flags[name] = tk.BooleanVar(value=False)

                # チェックONの天体のみ取得
                visible_names = None
                if hasattr(self, 'visible_object_flags') and self.visible_object_flags:
                    visible_names = [name for name, var in self.visible_object_flags.items() if var.get()]
                ...
                # タプルで、要素数3つあるか確認（dfi, found, df_all）
                ...
                if found > 0:
                    # カタログ選択に基づいて df_all をフィルタ
                    selected_types = [key for key, val in self.catalogs.items() if val.get_selected()]
                    df_filtered = df_all[df_all['TYPE'].isin(selected_types)].reset_index(drop=True)

                    if hasattr(self, 'object_control_window') and self.object_control_window.winfo_exists():
                        self.object_control_window.lift()
                        self.object_control_window.focus_force()
                    else:
                        self.show_object_selection_dialog(df_filtered)
    ...
    def show_object_selection_dialog(self, dataframe):
        ...
        # ========================
        # Select: All / None / Defaults
        # ========================
        select_frame = ttk.Frame(window)
        select_frame.pack(fill="x", pady=5, padx=0)

        ttk.Label(select_frame, text="Select:").pack(side="left")

        all_btn = ttk.Button(select_frame, text="All", command=lambda: self._select_objects(True))
        all_btn.pack(side="left", padx=2)

        none_btn = ttk.Button(select_frame, text="None", command=lambda: self._select_objects(False))
        none_btn.pack(side="left", padx=2)

        defaults_btn = ttk.Button(select_frame, text="Defaults", command=self._reset_object_defaults)
        defaults_btn.pack(side="left", padx=2)
        ...
        for i, row in enumerate(dataframe.itertuples(index=False)):
            ...
            check = ttk.Checkbutton(scrollable_frame, variable=self.visible_object_flags[name])
            check.grid(row=i, column=0, sticky="w", padx=2)

            name_label = ttk.Label(scrollable_frame, text=name, width=18)
            name_label.grid(row=i, column=1, sticky="w", padx=5, pady=2)

            catalog_label = ttk.Label(scrollable_frame, text=ctype, width=8)
            catalog_label.grid(row=i, column=2, sticky="w", padx=2)
            ...
            color_label.grid(row=i, column=4, padx=5)
            ...
            color_button.grid(row=i, column=3, padx=5)

        # ========================
        # Close ＆ ReApply ボタン（下部に固定）
        # ========================
        ...
        def reapply_with_confirmation():
            # window.destroy()  # ウィンドウは閉じておく
            self.apply_changes(from_cli=False, is_reapply=True)  # ここでフラグを渡す
        ...
        btn_c = ttk.Button(bottom_frame, text="C", width=2, command=lambda: self.switch_image("combined"))
        btn_c.pack(side="left", padx=2)

        btn_o = ttk.Button(bottom_frame, text="O", width=2, command=lambda: self.switch_image("overlay"))
        btn_o.pack(side="left", padx=2)

        btn_t = ttk.Button(bottom_frame, text="T", width=2, command=lambda: self.switch_image("table"))
        btn_t.pack(side="left", padx=2)

        btn_n = ttk.Button(bottom_frame, text="N", width=2, command=lambda: self.switch_image("original"))
        btn_n.pack(side="left", padx=2)

    def _select_objects(self, state: bool):
        """
        全天体の表示フラグを一括で state(True=ON/False=OFF) に設定し、
        チェックボックスの状態を更新する。
        """
    ...
    def _reset_object_defaults(self):
        """
        デフォルト設定に基づいて、全天体の表示フラグのみをリセットする。
        色は変更しない。
        """
        for name, var in self.visible_object_flags.items():
            # デフォルトは非表示
    ...
    try:
        if args.output is not None:
            ...
            if not siril.is_image_loaded():
                ...
                siril.disconnect()  # ← 忘れずに切断！
                return

            try:
                siril.pix2radec(0, 0)  # プレートソルブされているか確認
            except ValueError:
                messagebox.showerror("Plate Solving Error", "現在開いている画像はプレートソルブされていません。\nスクリプトを終了します。")
                siril.disconnect()  # ← 忘れずに切断！
                return

            siril.disconnect()  # ← 通常起動前にも明示的に切断しておくと安心
