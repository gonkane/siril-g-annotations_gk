...
- Objectウィンドウの採番制御。表示のON/−切替では No を即時更新せず、ReApply と 天体標本作成 の実行時に再採番
- ラベル配置の最適化。8方向 × 2半径（16候補）からコスト最小配置。衝突は無限大、画面外は大、天体楕円交差は中、距離は小の重み
- 楕円注釈。Simbad の galdim_majaxis、galdim_minaxis、galdim_angle に対応して銀河を楕円で描画
- CSV順保持。Stars、Messier、IC、NGC は CSV の行順を表示順と No に反映
- Export CSV の列名を日本語化。Name と TYPE を含む。表示名の列は末尾へ移動
- Replace CSV は長径、短径、回転角度も含めて内容をそのまま反映（並べ替えなし）
- Table 画像は dpi=200 の固定。結合画像作成時のサイズ不一致に追従して結合を安定化
- 進捗表示。結合画像およびテーブル生成の進行状況を Siril コンソールへ逐次表示
- WCSガード。プレートソルブ未実施時は一度だけ警告して安全終了
- 表示正規化。ロバスト百分位によるトーン調整で白飛びを抑制
- UI調整。11行表示のレイアウト、C/O/T/N ボタン位置の整理
...
最新: Galaxy_Annotations_102gk5.py
- No の再採番タイミングを ReApply と 標本作成 に変更
- ラベルの 16 候補からコスト最小配置（衝突、画面外、楕円交差、距離）
- Simbad の長径、短径、回転角度に対応し楕円描画
- Stars、Messier、IC、NGC の CSV 行順を表示順と番号へ反映
- Export CSV の列名を日本語化（Name、TYPE を含む）。表示名列は末尾
- Replace CSV は 長径、短径、回転角度 を含めそのまま反映
- Table 画像は dpi=200 固定。結合時のサイズ不一致にも対応
- 進捗ログ、WCS ガード、表示正規化、UI 調整（11行、C/O/T/N 整理）
