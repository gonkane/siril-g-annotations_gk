...
---
(注意）この改良版スクリプトは本来の「Galaxy Annotationsスクリプト」の

　　　選択したカタログの「銀河」のみ
   
  を抽出してアノテーションするという機能から
  
  　「Stars、Messier、NGC、IC」の選択したカタログの天体すべて　＋　選択した他カタログの「銀河」
   
  を抽出してアノテーションするという機能に変わっていますので、ご注意ください。
  
## 追加機能一覧

- gonkaneのパソコンの環境下では一部の天体のアノテーション丸の大きさがやや不自然だったため、
  Siril内に天体データが内蔵されている Messier、NGC、IC の３つのカタログは Siril の天体データから取得に切り替え
- それに伴って、Messier、NGC、IC の３つのカタログについては、銀河に限らず、すべてアノテーションするスクリプトに変更
...
### 4. 星の位置情報が付与され、ストレッチされた画像を開く

このスクリプトは赤経・赤緯（RA/DEC）情報をもとに天体の位置を特定するため、  
必ずSirilのアストロメトリーなどで星の位置情報が付与された画像を使用してください。
画像はストレッチを行い保存された画像を使ってください。
（注意）必ず星の位置情報を付与＆ストレッチを行ったあと、保存してからスクリプトを実行してください。
　　　　これは、スクリプトを繰り返し実行しなが好みの設定にする調整を可能にするための処置です。
